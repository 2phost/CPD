...
int initWorld(int world_size){
	...
	#pragma omp parallel for private(i, j, z) schedule(dynamic, world_size/8)
...
#pragma omp parallel for private(x, y, ate, aux, lowest_breeding) schedule(dynamic, size/8)
	for(x=0; x<size; x++){
		for(y=0 ; y<size; y++){
...
#pragma omp parallel sections
{
	#pragma omp section
	{
		...
		#pragma omp parallel for private(i, j) schedule(dynamic, size/8)
		for(i=0; i<size; i++){
